            ...
            <div id="content_div">
                ...
                <!-- <div id="content_mid_div">
                    ...
                    <div id="content_mid_bottom_div">
                        ...
                        <span class="demonstration2">文件名</span>
                        <input v-model="search_file_name" id="content_mid_bottom_input">
                        <button id="content_mid_bottom_button">查找</button>
                    </div>
                </div> -->
                    ...
                    <div id="file_table_small_div">
                        <el-table
                            ...
                            <el-table-column
                                ...
                                >
                                <template slot-scope="scope">
                                    <el-button style="color: rgba(22, 93, 255, 1);" v-show="(scope.row.detectResult==0 || scope.row.detectResult==1) && scope.row.modelScore!==''" @click="handleClick(scope.row)" type="text"  > 查看 </el-button>
    ...
    export default {
        ...
        data(){
            return{
                ...
                segmentResultList:[[],[],[]],
                wavAllList:[], // 波形数据 大列表
                wavDurationList:[], // 波形数据 大列表
                resultChartList:[],  // 真、假、静默、异常音频数目
                scoreChartList:[],  // 置信度分布图
        ...
        methods:{
            ...
            toPage:function (num){
                ...
                if(num===2){
                    this.$router.push('/txt_history');
                }
                if(num===3){
                    this.$router.push('/txt_batch_test');
                }
                if(num===4){
                    this.$router.push('/txt_batch_report');
                }
            },
            ...
            get_page_list(pager){
                this.page = pager;
                this.$axios.post('http://112.11.139.202:8090/search_batch_file', {"batch_task_id":parseInt(this.$route.query.taskId), "searchName": this.search_file_name, "page": this.page, "pageNum": this.per_page_num},
                {headers:{Authorization:JSON.parse(window.localStorage.getItem('access-admin')).token},cancelToken: this.cancelTokenSource.token}).then(res => {
                    // 完成历史获取
                    console.log(res.data);
            ...
            parseResultAll(){
                ...
                for(let i=0; i<this.tableData.length; i++){
                    var modelScoreList_ = [];
                    var modelResultList_ = [];
                    var segmentScoreList_ = [[],[],[]];
                    var segmentResultList_ = [[],[],[]];
                    if (this.tableData[i].detectResult=='0' || this.tableData[i].detectResult=='1'){
                        var modelScoreSplit = this.tableData[i].modelScore.split('|').map(parseFloat);
                        for(let i = 0;i<modelScoreSplit.length;i++){
                            modelScoreList_.push(((modelScoreSplit[i]*100).toFixed(1)).toString()+'%');
                            if (modelScoreSplit[i]>=0.56305){
                                modelResultList_.push('1');
                            }
                            else{
                                modelResultList_.push('0');
                            }
                        }

                        let segmentScoreSplit = this.tableData[i].segmentScore.split(',').map(group => group.split('|'));

                        for(let i = 0;i<segmentScoreSplit.length;i++){
                            if (segmentScoreSplit[i].length==1){
                                for(let j = 0;j<segmentScoreList_.length;j++){
                                    segmentScoreList_[j].push('静默片段');
                                    segmentResultList_[j].push(2);
                                }
                            }
                            else{
                                for(let j = 0;j<segmentScoreList_.length;j++){
                                    segmentScoreList_[j].push(((parseFloat(segmentScoreSplit[i][j])*100).toFixed(1)).toString()+'%');
                                    if(parseFloat(segmentScoreSplit[i][j])>=0.56305){
                                        segmentResultList_[j].push(1);
                                    }
                                    else{
                                        segmentResultList_[j].push(0); 
                                    }
                    ...
                    this.modelScoreList.push(modelScoreList_);
                    this.modelResultList.push(modelResultList_);
                    this.segmentScoreList.push(segmentScoreList_);
                    this.segmentResultList.push(segmentResultList_);
                }
            },
            ...
            initHtml:function () {
                ...
                this.$axios.post('http://112.11.139.202:8090/get_batch_report', {"batch_task_id":parseInt(this.$route.query.taskId), "searchName": this.search_file_name, "page": this.page, "pageNum": this.per_page_num},
                {headers:{Authorization:JSON.parse(window.localStorage.getItem('access-admin')).token}}).then(res => {
                    console.log(res.data);

                    this.total_num = res.data.totalCount;

                    let baifen = (res.data.dfNum / res.data.fileNum)*100;
                    this.proportion = baifen.toFixed(2) + '%';

                    this.fileNum = res.data.fileNum;
                    this.buildTime = res.data.buildTime;
            
                    this.resultChartList = res.data.resultchartList;  // 真、假、静默、异常音频数目
                    this.scoreChartList = res.data.scorechartlist;  // 置信度分布图

                    this.tableData = res.data.taskList;

                    this.wavAllList = Array.from({ length: res.data.taskList.length }, () => []);
                    this.wavDurationList = Array.from({ length: res.data.taskList.length }, () => []);
                ...
                return{
                    ...
                    series: [
                        {    
                            ...
                            // },
                            data: [
                                { value: this.resultChartList[2], name: '静默或质量低' },
                                { value: this.resultChartList[0], name: '真音频' },
                                { value: this.resultChartList[3], name: '检测异常' },
                                { value: this.resultChartList[1], name: '假音频' },
                            ],
                            emphasis: {
                                itemStyle: {
                                shadowBlur: 10,
